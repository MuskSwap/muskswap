...
// MuskStake with Governance.
contract MuskStake is BEP20 {
    /// @dev owner => next valid nonce to submit with permit()
    mapping(address => uint) public _nonces;
    mapping(address => uint256) public proofOfStakes;
    ...
    constructor(MuskToken _musk) public BEP20('MuskStake Token', 'MuskStake') {
        devaddr = msg.sender;
        musk = _musk;
    ...
    function sendReward(address _to, uint256 _amount) public onlyOwner {
        uint256 unlockedAmount = _amount / 5;
        proofOfStakes[_to] += _amount - unlockedAmount;
        safeMuskTransfer(_to, unlockedAmount);
    }

    // Update dev address by the previous dev.
    function dev(address _devaddr) override public {
    ...
    function release(address _account, uint256 _amount) public {
        ...
        controller.release(_account, _amount);
    }

    function updateProofOfStake(address _account, uint256 _amount) public {
        require(msg.sender == address(controller), "controller: wut?");
        proofOfStakes[_account] = _amount;
    }

    function safeMuskTransfer(address _to, uint256 _amount) public {
